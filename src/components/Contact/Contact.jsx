...
export default function Contact() {
  ...
  return (
    <section id="contact" className="section bg-[var(--bg-3)] relative">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-8">
        ...
        <div ref={ref} className={`text-center mb-16 reveal ${visible ? 'visible' : ''}`}>
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold text-[var(--text)] mb-4">Contact Me</h2>
        ...
        <div ref={ref} className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="glass-card p-8">
              <h3 className="text-[var(--text)] font-bold text-base mb-6">Contact Information</h3>
              <div className="space-y-5">
                {contactRows.map(({ icon, label, value, href }) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-[var(--text-dim)] text-[10px] uppercase tracking-wider mb-0.5">{label}</p>
                      <p className="text-[var(--text)] text-sm font-medium group-hover:text-gold transition-colors">{value}</p>
                    </div>
                  </a>
                ))}
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/[0.05]">
                <p className="text-[var(--text-dim)] text-[10px] uppercase tracking-wider mb-4">Follow Me</p>
            ...
            <div className="glass-card p-5 flex items-center gap-4">
              ...
              <p className="text-[var(--text)] text-sm font-medium">{personalInfo.availability}</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3">
            <div className="glass-card p-8 md:p-10">
              <h3 className="text-[var(--text)] font-bold text-base mb-6">Send a Message</h3>
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-[var(--text-dim)] text-[10px] uppercase tracking-wider font-medium mb-2">Your Name</label>
                    <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="Najib Ullah Khan" required
                      className="w-full bg-[var(--bg-4)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] text-sm outline-none focus:border-gold/40 transition-colors placeholder:text-[var(--text-dim)]" />
                  </div>
                  <div>
                    <label className="block text-[var(--text-dim)] text-[10px] uppercase tracking-wider font-medium mb-2">Email</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-[var(--text-dim)] text-[10px] uppercase tracking-wider font-medium mb-2">Phone</label>
                    <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="+92 344 658 0048" required
                      className="w-full bg-[var(--bg-4)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] text-sm outline-none focus:border-gold/40 transition-colors placeholder:text-[var(--text-dim)]" />
                  </div>
                  <div>
                    <label className="block text-[var(--text-dim)] text-[10px] uppercase tracking-wider font-medium mb-2">Subject</label>
                    <input type="text" name="subject" value={form.subject} onChange={handleChange} placeholder="Project Inquiry"
                      className="w-full bg-[var(--bg-4)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] text-sm outline-none focus:border-gold/40 transition-colors placeholder:text-[var(--text-dim)]" />
                  </div>
                </div>
                <div>
                  <label className="block text-[var(--text-dim)] text-[10px] uppercase tracking-wider font-medium mb-2">Message</label>
                  <textarea name="message" value={form.message} onChange={handleChange} placeholder="Tell me about your project..." rows="5" required
                    className="w-full bg-[var(--bg-4)] border border-[var(--border)] rounded-xl px-4 py-3 text-[var(--text)] text-sm outline-none focus:border-gold/40 transition-colors placeholder:text-[var(--text-dim)] resize-none" />
                </div>
